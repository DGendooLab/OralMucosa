...
if (is.null(input)){
  seurat_filtered_list <- readRDS(file.path(cache_dir, paste0("NormAndScaled_VU40T_HTK_HGK_Seurat_filtered_individual_samples_list_", species,".rds")))
  } else {
...
saveRDS(seurat_filtered_list, file = file.path(cache_dir, paste0("preDoubletFinder_VU40T_HTK_HGK_seuratList_",species,".rds")))
...
##run if crash
#seurat_filtered_list <- readRDS(file.path(cache_dir, paste0("preDoubletFinder_VU40T_HTK_HGK_seuratList_",species,".rds")))
...
saveRDS(seurat_filtered_list, file = file.path(cache_dir, paste0("DoubletFinder_results_VU40T_HTK_HGK_",species,".rds")))

seurat_filtered_list <- readRDS(file.path(cache_dir, paste0("DoubletFinder_results_VU40T_HTK_HGK_",species,".rds")))
...
saveRDS(seurat_singlets_list, file = file.path(cache_dir, paste0("VU40T_HTK_HGK_singlets_only_sep_samples_",species,".RDS")))

message("Doublets removed from samples successfully \n
        RDS checkpoint saved to: ", file.path(cache_dir, paste0("VU40T_HTK_HGK_singlets_only_sep_samples_",species,".RDS")))
